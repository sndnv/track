defmodule Cli.HelpTest do
  ...
  test "strips margin" do
    ...
    actual_string = Cli.Help.strip_margin(original_string)

    assert actual_string == expected_string
  end

  test "generates a bash_completion script" do
    test_app = "test_help"
    ...
    actual_result =
      Cli.Help.generate_bash_completion_script_from_attributes(
        test_app,
        test_commands,
        test_additional_options
      )
    ...
    expected_result = [
      "# Automatically generated by `test_help`",
      "_test_help()",
      "{",
      ...
      "",
      "  opts=\"list report start stop --config --verbose\"",
      "",
      "  case \"${command}\" in",
      "    list)",
      "      if [[ \"${prev}\" == \"list\" ]] ; then",
      "        COMPREPLY=( $(compgen -W \"--from --to --sort-by --order\" -- ${cur}) )",
      "      else",
      "        declare -a opts=(--from --to --sort-by --order)",
      ...
      "    report)",
      "      if [[ \"${prev}\" == \"report\" ]] ; then",
      "        COMPREPLY=( $(compgen -W \"duration day week month task overlap\" -- ${cur}) )",
      "      else",
      "        declare -a opts=(--from --to --sort-by --order)",
      "        declare -a filtered_opts=()",
      "        for word in \"${opts[@]}\"; do [[ ! \" ${COMP_WORDS[@]} \" =~ \" ${word} \" ]] && filtered_opts+=(${word}); done",
      ...
      "  return 0",
      "}",
      "complete -F _test_help test_help",
      ""
    ]
